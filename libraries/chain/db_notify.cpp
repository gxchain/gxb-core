...
struct get_impacted_account_visitor
{
   ...
   void operator()( const account_upgrade_merchant_operation& op ) {}
   void operator()( const account_upgrade_datasource_operation& op ) {}
   void operator()( const account_upgrade_data_transaction_member_operation& op ) {}
   ...
   // stale operator()
   void operator()( const stale_data_market_category_create_operation& op ) { }
   void operator()( const stale_data_market_category_update_operation& op ) { }
   void operator()( const stale_free_data_product_create_operation& op ) { }
   void operator()( const stale_free_data_product_update_operation& op ) { }
   void operator()( const stale_league_data_product_create_operation& op ) { }
    ...
    * @param op
    */
   void operator()( const data_market_category_create_operation& op )
   {
      _impacted.insert( op.issuer );
   }
   void operator()( const data_market_category_update_operation& op )
   {
   }
   void operator()( const free_data_product_create_operation& op )
   {
      _impacted.insert( op.issuer );
   }
   void operator()( const free_data_product_update_operation& op )
   {
   }
   void operator()( const league_data_product_create_operation& op )
   {
      _impacted.insert( op.issuer );
   }
   void operator()( const league_data_product_update_operation& op )
   {
   }
   void operator()( const league_create_operation& op )
   {
      _impacted.insert( op.issuer );
   }
   void operator()( const league_update_operation& op )
   {
   }
   ...
   void operator()( const pay_data_transaction_operation& op )
   {
      _impacted.insert( op.from );
   }
   ...
   void operator() (const datasource_copyright_clear_operation& op) {}
   ...
   void operator() (const contract_js_deploy_operation& op) {
       _impacted.insert(op.account);
   }

   void operator() (const contract_js_call_operation& op) {
       _impacted.insert(op.account);
       _impacted.insert(op.contract_id);
   }
};
       ...
       switch ((impl_object_type) obj->id.type()) {
           ...
           case impl_account_balance_object_type: {
             ...
             case impl_fba_accumulator_object_type:
              break;
             case impl_account_merchant_object_type:
              break;
             case impl_free_data_product_search_results_object_type:
              break;
             case impl_league_data_product_search_results_object_type:
              break;
...
{ try {
   if( _undo_db.enabled() ) {
      // Changed
      if (!data_transaction_changed_objects.empty()) {
          string notify_request_id;
          for (const auto& op : trx.operations) {
              // idump((trx));
              if (op.which() == operation::tag<data_transaction_create_operation>::value) {
                  notify_request_id = op.get<data_transaction_create_operation>().request_id;
                  // dlog("data_transaction_create_operation");
              }
              else if (op.which() == operation::tag<data_transaction_update_operation>::value) {
                  notify_request_id = op.get<data_transaction_update_operation>().request_id;
                  // dlog("data_transaction_update_operation");
              }
              else if (op.which() == operation::tag<data_transaction_datasource_upload_operation>::value) {
                  notify_request_id = op.get<data_transaction_datasource_upload_operation>().request_id;
                  // dlog("data_transaction_datasource_upload_operation");
              }
              else if (op.which() == operation::tag<data_transaction_datasource_validate_error_operation>::value) {
                  notify_request_id = op.get<data_transaction_datasource_validate_error_operation>().request_id;
                  // dlog("data_transaction_datasource_validate_error_operation");
              }
              else if (op.which() == operation::tag<pay_data_transaction_operation>::value) {
                  notify_request_id = op.get<pay_data_transaction_operation>().request_id;
                  // dlog("pay_data_transaction_operation");
              }
              // idump((notify_request_id));
          }
          if (!notify_request_id.empty()) {
              data_transaction_changed_objects(notify_request_id);
          }
      }
   }
} FC_CAPTURE_AND_LOG( (0) ) }
