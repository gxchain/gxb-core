...
void query_txid_plugin::plugin_set_program_options(
    ...
    cli.add_options()
        ("query-txid-path", boost::program_options::value<std::string>(), "Save the leveldb path of the transaction history")
        ("limit-batch", boost::program_options::value<uint64_t>(), "Number of records written to leveldb in batches")
        ;
    cfg.add(cli);
}
