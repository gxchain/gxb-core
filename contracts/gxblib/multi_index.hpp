...
class multi_index
{
   private:
      ...
      struct index {
         public:
            ...
            struct const_iterator : public std::iterator<std::bidirectional_iterator_tag, const T> {
               public:
                  ...
                  const_iterator& operator++() {
                     ...
                     // gxb_assert( _item != nullptr, "cannot increment end iterator" );
                  ...
                  const_iterator& operator--() {
                     ...
                     if( !_item ) {
                        ...
                        // gxb_assert( ei != -1, "cannot decrement end iterator when the index is empty" );
                        prev_itr = secondary_index_db_functions<secondary_key_type>::db_idx_previous( ei , &prev_pk );
                        // gxb_assert( prev_itr >= 0, "cannot decrement end iterator when the index is empty" );
                     } else {
                        if( _item->__iters[Number] == -1 ) {
                           secondary_key_type temp_secondary_key;
                           auto idxitr = secondary_index_db_functions<secondary_key_type>::db_idx_find_primary(_idx->get_code(), _idx->get_scope(), _idx->name(), _item->primary_key(), temp_secondary_key);
                        ...
                        // gxb_assert( prev_itr >= 0, "cannot decrement iterator at beginning of index" );
                     }
            ...
            const_iterator iterator_to( const T& obj ) {
               ...
               // gxb_assert( objitem.__idx == _multidx, "object passed to iterator_to is not in multi_index" );
            ...
            void modify( const_iterator itr, uint64_t payer, Lambda&& updater ) {
               // gxb_assert( itr != cend(), "cannot pass end iterator to modify" );

               _multidx->modify( *itr, payer, std::forward<Lambda&&>(updater) );
            }

            const_iterator erase( const_iterator itr ) {
               // gxb_assert( itr != cend(), "cannot pass end iterator to erase" );
      ...
      const item& load_object_by_primary_iterator( int32_t itr )const {
         ...
         // gxb_assert( size >= 0, "error reading iterator" );
   ...
   public:
      ...
      struct const_iterator : public std::iterator<std::bidirectional_iterator_tag, const T> {
         ...
         const_iterator& operator--() {
            ...
            if( !_item ) {
               ...
               // gxb_assert( ei != -1, "cannot decrement end iterator when the table is empty" );
               prev_itr = db_previous_i64( ei , &prev_pk );
               // gxb_assert( prev_itr >= 0, "cannot decrement end iterator when the table is empty" );
            } else {
               prev_itr = db_previous_i64( _item->__primary_itr, &prev_pk );
               // gxb_assert( prev_itr >= 0, "cannot decrement iterator at beginning of table" );
            }
      ...
      uint64_t available_primary_key()const {
         ...
         // gxb_assert( _next_primary_key < no_available_primary_key, "next primary key in table is at autoincrement limit");
      ...
      const_iterator iterator_to( const T& obj )const {
         const auto& objitem = static_cast<const item&>(obj);
         // gxb_assert( objitem.__idx == this, "object passed to iterator_to is not in multi_index" );
         return {this, &objitem};
      }
      ...
      const_iterator emplace( uint64_t payer, Lambda&& constructor ) {
         ...
         // gxb_assert( _code == current_receiver(), "cannot create objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
      ...
      template<typename Lambda>
      void modify( const_iterator itr, uint64_t payer, Lambda&& updater ) {
         // gxb_assert( itr != end(), "cannot pass end iterator to modify" );
      ...
      void modify( const T& obj, uint64_t payer, Lambda&& updater ) {
         ...
         const auto& objitem = static_cast<const item&>(obj);
         // gxb_assert( objitem.__idx == this, "object passed to modify is not in multi_index" );
         auto& mutableitem = const_cast<item&>(objitem);
         // gxb_assert( _code == current_receiver(), "cannot modify objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
         ...
         // gxb_assert( pk == obj.primary_key(), "updater cannot change primary key when modifying an object" );
   ...
   const T& get( uint64_t primary, const char* error_msg = "unable to find key" )const {
         ...
         // gxb_assert( result != cend(), error_msg );
         return *result;
      }
      ...
      const_iterator erase( const_iterator itr ) {
         // gxb_assert( itr != end(), "cannot pass end iterator to erase" );
      ...
      void erase( const T& obj ) {
         ...
         const auto& objitem = static_cast<const item&>(obj);
         // gxb_assert( objitem.__idx == this, "object passed to erase is not in multi_index" );
         // gxb_assert( _code == current_receiver(), "cannot erase objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
         ...
         // gxb_assert( itr2 != _items_vector.rend(), "attempt to remove object that was not in multi_index" );
