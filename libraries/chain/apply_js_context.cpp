...
void apply_js_context::exec_one()
{
    ...
    try {
        ...
        try {
            v8::V8::InitializeICUDefaultLocation("./witness_node");
            v8::V8::InitializeExternalStartupData("./witness_node");
            ...
            v8::Local<v8::String> source = v8::String::NewFromUtf8(isolate,
                                                                   std::string(contract_obj.code.begin(), contract_obj.code.end()).c_str(),
            ...
            dlog("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxx result = ${x}", ("x", *utf8));
    ...
    if (contract_log_to_console) {
        ...
        auto prefix = fc::format_string(
            "\n[(${a},${n})->${r}] ",
            fc::mutable_variant_object()("a", act.contract_id)("n", std::string(act.method_name))("r", receiver));
...
void apply_js_context::reset_console()
{
    if (contract_log_to_console) {
...
void apply_js_context::execute_inline(action &&a)
{
...
int apply_js_context::db_store_i64(uint64_t scope, uint64_t table, const account_name &payer, uint64_t id, const char *buffer, size_t buffer_size)
{
    return db_store_i64(receiver, scope, table, payer, id, buffer, buffer_size);
}

int apply_js_context::db_store_i64(uint64_t code, uint64_t scope, uint64_t table, const account_name &payer, uint64_t id, const char *buffer, size_t buffer_size)
{
...
void apply_js_context::db_update_i64(int iterator, account_name payer, const char *buffer, size_t buffer_size)
{
...
void apply_js_context::db_remove_i64(int iterator)
{
    const key_value_object &obj = keyval_cache.get(iterator);
...
int apply_js_context::db_get_i64(int iterator, char *buffer, size_t buffer_size)
{
    const key_value_object &obj = keyval_cache.get(iterator);
...
int apply_js_context::db_next_i64(int iterator, uint64_t &primary)
{
...
int apply_js_context::db_previous_i64(int iterator, uint64_t &primary)
{
...
int apply_js_context::db_find_i64(uint64_t code, uint64_t scope, uint64_t table, uint64_t id)
{
...
int apply_js_context::db_lowerbound_i64(uint64_t code, uint64_t scope, uint64_t table, uint64_t id)
{
    const auto *tab = find_table(code, scope, table);
    if (!tab) return -1;
...
int apply_js_context::db_upperbound_i64(uint64_t code, uint64_t scope, uint64_t table, uint64_t id)
{
    const auto *tab = find_table(code, scope, table);
    if (!tab) return -1;
...
int apply_js_context::db_end_i64(uint64_t code, uint64_t scope, uint64_t table)
{
    const auto *tab = find_table(code, scope, table);
    if (!tab) return -1;
...
const table_id_object *apply_js_context::find_table(uint64_t code, name scope, name table)
{
...
const table_id_object &apply_js_context::find_or_create_table(uint64_t code, name scope, name table, const account_name &payer)
{
    const auto &table_idx = _db->get_index_type<table_id_multi_index>().indices().get<by_code_scope_table>();
    auto existing_tid = table_idx.find(boost::make_tuple(code, scope, table));
    if (existing_tid != table_idx.end()) {
...
void apply_js_context::remove_table(const table_id_object &tid)
{
    _db->remove(tid);
}
